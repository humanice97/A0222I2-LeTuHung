...
select kh.ma_khach_hang, count(kh.ma_khach_hang) as so_lan_dat_phong , kh.ho_ten 
...
-- Hiển thị ma_khach_hang, ho_ten, ten_loai_khach, ma_hop_dong, ten_dich_vu, ngay_lam_hop_dong, ngay_ket_thuc, tong_tien (Với
...
select ma_dich_vu from hop_dong where ngay_lam_hop_dong between '2021-01-01' and '2021-03-30')
...
-- so_nguoi_toi_da, chi_phi_thue, ten_loai_dich_vu của tất cả các loại dịch vụ đã từng được khách hàng đặt phòng 
...
(select ma_dich_vu from hop_dong where year(hop_dong.ngay_lam_hop_dong) = '2020' group by ma_dich_vu) 
and dv.ma_dich_vu not in 
(select ma_dich_vu from hop_dong where year(hop_dong.ngay_lam_hop_dong) = '2021' group by ma_dich_vu)
group by ma_dich_vu;


-- Cau 8
-- Hiển thị thông tin ho_ten khách hàng có trong hệ thống, với yêu cầu ho_ten ko trùng nhau.
...
where kh.ma_loai_khach = '1' and kh.dia_chi regexp 'Vinh|Quảng Ngãi';
...
-- từng được khách hàng đặt vào 3 tháng cuối năm 2020 nhưng chưa từng được khách hàng đặt vào 6 tháng đầu năm 2021.
 ...
 select hd.ma_hop_dong, nv.ho_ten as ho_ten_nhan_vien, kh.ho_ten as ho_ten_khach_hang, kh.so_dien_thoai as sdt_khach_hang,dv.ma_dich_vu,
 ...
 from hop_dong hd
  ...
  select ma_hop_dong 
  from hop_dong 
  where month(ngay_lam_hop_dong) between 10 
  and 12 and year(ngay_lam_hop_dong) = '2020'
  )
  and hd.ma_hop_dong not in (
  select ma_hop_dong 
  from hop_dong 
  where month(ngay_lam_hop_dong) between 1 
  and 6 and year(ngay_lam_hop_dong) = '2021'
  )
  group by ma_hop_dong;
...
select dvdk.ma_dich_vu_di_kem, dvdk.ten_dich_vu_di_kem, sum(so_luong) as sl_dich_vu_di_kem
from dich_vu_di_kem dvdk
 join hop_dong_chi_tiet hdct on dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
 join hop_dong hd on hd.ma_hop_dong = hdct.ma_hop_dong
...
-- Hiển thị thông tin tất cả các Dịch vụ đi kèm chỉ mới được sử dụng một lần duy nhất. Thông tin hiển thị bao gồm ma_hop_dong,
...
select lk.ma_loai_khach as ma_loai_khach_update , ifnull (( ifnull (dv.chi_phi_thue, 0) + ifnull (sum(hdct.so_luong * dvdk.gia),0 ) ),0) as tong_tien
from khach_hang kh
...
(
select ma_loai_khach_update
from update_khach_hang
);
...
create view update_gia as
select dvdk.ma_dich_vu_di_kem, count(dvdk.ma_dich_vu_di_kem) as so_lan_su_dung
from hop_dong hd 
join hop_dong_chi_tiet hdct on hdct.ma_hop_dong = hd.ma_hop_dong
join dich_vu_di_kem dvdk on dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
